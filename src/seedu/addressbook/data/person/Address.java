...
public class Address {
	...
	public void splitAddress() {
		String[] subclasses = new String[4];
		int i = 0;
		for (String subAddress : value.split(",")) {
			subclasses[i] = subAddress;
			subclasses[i] = subclasses[i].trim();
			++i;
		}
		block = new Block(subclasses[0]);
		street = new Street(subclasses[1]);
		unit = new Unit(subclasses[2]);
		postalcode = new Postalcode(subclasses[3]);
